function CReelColumn(iIndex,iXPos,iYPos,iDelay,aSymbols,oParentContainer){
    ...
    this._init = function(iIndex,iXPos,iYPos,iDelay,aSymbols){
        ...
        _iFinalY = _iCurStartY + ( (SYMBOL_HEIGHT + SPACE_HEIGHT_BETWEEN_SYMBOLS) * NUM_ROWS)-OFFSET_Y_SYMBOLS;
    ...
    this.initContainer = function(iXPos,iYPos,aSymbols){
        ...
        var iX = 0;
        var iY = -OFFSET_Y_SYMBOLS;
        _aSymbols = new Array();
        _aSymbolValues = new Array();
      
        for(var i=0;i<NUM_ROWS;i++){
             var oSprite = createSprite(s_aSymbolData[aSymbols[i]], "static",0,0,SYMBOL_WIDTH,SYMBOL_HEIGHT);
             oSprite.stop();
             oSprite.x = iX;
             oSprite.y = iY;
    ...
    this._setSymbol = function(aSymbols){
        ...
        for(var i=0;i<aSymbols.length;i++){
            _aSymbols[i].spriteSheet = s_aSymbolData[aSymbols[i]];
